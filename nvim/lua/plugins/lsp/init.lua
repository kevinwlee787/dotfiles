...
function M.config()
  ...
  local servers = {
    clangd = {
      cmd = {
          -- update to absolute path if clangd is not in PATH
          'clangd',
          '--header-insertion=never', -- don't auto-insert #include lines
          '--pch-storage=memory',     -- keep preambles in RAM instead of /tmp
          '-j=32',                    -- parallel background indexing workers
          '--malloc-trim',            -- release freed memory back to OS
          '--limit-results=2000',     -- max completion candidates (default 100)
        },
